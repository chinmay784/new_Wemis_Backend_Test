...
function ManufactureList() {
    ...
    const fetchManufacturers = async () => {
        try {
            const response = await axios.post(
                "http://localhost:4000/api/wlp/fetchManuFactur",
                {},
    ...
    const handleDelete = async (id) => {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                try {
                    const response = await axios.post(
                        "http://localhost:4000/api/wlp/deleteManuFactur",
                        { manufacturId: id },
                        {
                            headers: {
    ...
    const handleEditClick = async (id) => {
        ...
        try {
            const response = await axios.post(
                "http://localhost:4000/api/wlp/findManuFacturById",
                { manufacturId: id },
                { headers: { Authorization: `Bearer ${tkn}` } }
            );
    ...
    const handleSave = async (e) => {
        ...
        try {
            const response = await axios.post(
                "http://localhost:4000/api/wlp/editManuFactur",  // ✅ updated endpoint
