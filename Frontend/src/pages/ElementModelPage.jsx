...
function ElementModelPage() {
  ...
  const fetchData = async () => {
    try {
     ...
     setData(response.data.allFetchModelElementData)
  ...
  const handleDelete = async (id) => {
    try {
      await axios.post(
        "http://localhost:4000/api/superadmin/deleteElement",
        { id },
        {
          headers: {
            Authorization: `Bearer ${tkn}`,
            ...
            <tr className="bg-yellow-400 text-black text-sm uppercase tracking-wide">
              ...
              <th className="py-3 px-6 text-left">Type</th>
                <th className="py-3 px-6 text-left">Model No</th>
                   <th className="py-3 px-6 text-left">Voltage</th>
              ...
              filteredData.map((item, index) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 text-lg">{item.elementName || "N/A"}</td>
                   ...
                   <td className="py-4 px-6 text-lg">{item.voltage }</td>
...
export default ElementModelPage;
