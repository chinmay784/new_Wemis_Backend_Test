...
function AdminList() {
  ...
  const fetchAdmins = async () => {
    try {
      const response = await axios.post(
        "http://localhost:4000/api/superadmin/getAllAdmins",
        {},
  ...
  const handleUpdate = async (id, e) => {
    ...
    try {
      const response = await axios.post(
        "http://localhost:4000/api/superadmin/editAdmin",
        { adminId: id, ...formData },
        { headers: { Authorization: `Bearer ${tkn}` } }
      );
  ...
  const handleEdit = async (id) => {
    try {
      const response = await axios.post(
        "http://localhost:4000/superadmin/getAdminById",
        { adminId: id },
        {
          headers: {
  ...
  const handleDelete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          const response = await axios.post(
            "http://localhost:4000/api/superadmin/deleteAdmin",
            { adminId: id },
            {
              headers: {
                Authorization: `Bearer ${tkn}`,
